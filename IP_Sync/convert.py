...
def convert_ip(mysql_object, college_tablename, num_config, start_id, correct_list, correct_json) : #将IP数据库内的地址细分为省市区
    if start_id > 1 :
        start_id = start_id
    else:
        start_id = 1
    ...
    sql = mysql_object.query("select `id`,`address`,`location` from `iprange_info` limit " + str(start_id - 1) + "," + str(num_config))
    result_num = len(sql)
    i = 0
    addr_list = []
    for linelist in sql:
        ...
        addr = splitAddress( mysql_object, college_tablename, addr, linelist[ "location" ], correct_list, correct_json)
        addr.append(start_id + i)
        addr_list.append(addr)
        i += 1
    #print(addr_list)
    if addr_list:
        mysql_object.executemany( "update `iprange_info` set `country`= (%s) ,`province`= (%s) ,`city`= (%s) ,`area`= (%s), `location`=(%s)  where `id`= (%s) ",addr_list )
    else:
        print( "------------------------------------------- \n已全部完成转换。" )
        return
    ...
    if result_num == num_config:
        ...
        mysql_object.__del__
        convert_ip(mysql_object, college_tablename, num_config, next_id, correct_list, correct_json)
    else:
...
def splitAddress( mysql_object, college_tablename, address, location, correct_list, correct_json ): #从IP库的地址中提取省市区等数据
    country = ''
    province = ''
    city = ''
    area = ''
    ...
    if matches:
        ...
        size,college_list,college_dict = college(mysql_object, college_tablename, college_name)
        if size:
...
def college(mysql_object, college_tablename, college_name):
    college_name = '%%'+college_name + '%%'
    sql = mysql_object.query("select * from `"+ college_tablename + "` where `name` like '" + college_name+"'")
    name = []
    if sql:
        for line in sql:
...
def convert(mysql_object,college_tablename,num_config,start_id,college_filename,correct_filename):
...
if __name__ == '__main__':
    ...
    -------
    """
    opts,args = getopt.getopt(sys.argv[1:],'-h-n:-s:-t:-f:-c:',['help','num_config','start_id','college_tablename=','college_filename=','correct_filename='])
    varlist = []
    for opt_name,opt_value in opts:
        if opt_name in ("-h", "--help"):
            usage()
            sys.exit()
        if opt_name in ("-n", "--num_config"):
            num_config = int(opt_value)
            varlist.append('num_config')
        if opt_name in ("-s", "--start_id"):
            start_id = int(opt_value)
            varlist.append('start_id')
        if opt_name in ("-t", "--college_tablename"):
