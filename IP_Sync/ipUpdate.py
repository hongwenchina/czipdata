...
def datdown(filename,version_file):
    def get_fetcher():
    ...
    with open(version_file, "rb+") as handle:
        ...
        if len(content) > 0:
            ...
            print('本地数据文件版本: ' + str(curr_version))
    ...
    if version == curr_version:
        ...
        return 0
    upstart = '数据文件新版本: ' + str(version) + ', 大小: ' + str(size)
    logger.info(upstart)
    print(upstart)
    print( "------------------------------------------- \n " )
...
if __name__ == '__main__':
    ...
    ret = datdown(filename,version_file)
    if ret > 0:
        print('成功写入到%s, %s字节' %
              (filename, format(ret, ','))
            )
